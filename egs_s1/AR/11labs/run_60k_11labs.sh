...
stage=0
stop_stage=100
train_output_path=exp_11labs/60k_11labs_scratch
...
config_path=conf/60k_11labs_scratch.yaml
# 数据预处理的时候用这个
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    CUDA_VISIBLE_DEVICES=${gpus} ./local/train_60k_11labs.sh ${config_path} ${train_output_path} ${root_dir} ${dump_dir}|| exit -1
fi
